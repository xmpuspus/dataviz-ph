...
- `echarts-custom-5.6.0.min.js` — the build index.html loads. Tree-shaken
  ECharts 5.6.0 (629 KB raw / 210 KB gzip vs 1,031 KB / 334 KB full): only the
  modules app.js uses (scatter/line/bar/map charts; title, tooltip, grid,
  visualMap, timeline, markArea, geo, axisPointer components; LabelLayout;
  CanvasRenderer), re-exported as the `echarts` global.
...
npx esbuild entry.js --bundle --minify --format=iife --global-name=echarts \
  --outfile=../../public/vendor/echarts-custom-<version>.min.js
openssl dgst -sha384 -binary ../../public/vendor/echarts-custom-<version>.min.js | openssl base64 -A
```
